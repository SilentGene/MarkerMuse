...
* Supports **two scoring modes**:
  ...
  * **Weighted mode** — uses human-curated metric weights loaded from `weights.yaml` (you can edit this file to customize scoring).
  * **Fitted mode** — uses SingleM taxonomic-profile output to fit regression-based marker performance weights and rank markers according to how well they predict observed taxonomic coverage.
...
Weighted mode loads metric weights from `weights.yaml`. You can edit this file to adjust the influence of each metric. Lower-is-better metrics are inverted after min–max normalization (singleton_rate, slope_rarefaction, cv_shannon).
...
## 🧾 Customizing Weights (weighted mode)
